...
Процедура ПриГлобальномПоиске(ИзначальнаяСтрокаПоиска, ПланПоиска) Экспорт
	...
	Для Каждого КлючИЗначение Из ПараметрыПоиска.Команды Цикл
		...
		Для Каждого ЧастьСтроки Из ЧастиСтрокиПоиска Цикл
			...
			ПодчиненнаяКоманда = ТекущаяКоманда.Команды.Получить(ТекущиеКлючИПараметры.Ключ); // см. НоваяКомандаРасширенногоПоиска
...
Функция ПараметрыВыполненияКомандыПоиска(ПараметрыПоиска, СтрокаПоиска, Команда, КлючИПараметры) Экспорт
	...
	Если КлючИПараметры = Неопределено Тогда
		ПараметрыВыполнения.ПолныйКлючСПараметрами = ПараметрыВыполнения.ПолныйКлюч;
	Иначе
		ПараметрыВыполнения.ПолныйКлючСПараметрами = ПараметрыВыполнения.ПолныйКлюч	+ КлючИПараметры.СтрокаПараметров;
...
Функция ПараметрыРасширенногоГлобальногоПоиска(ИзКэша = Истина) Экспорт
	...
	ЗаполнитьСтандратныеПараметрыКоманд(ПараметрыПоиска);
...
Функция СтрокаПоискаЗаВычетомКоманды(ПараметрыВыполнения, СтрокаПоиска) Экспорт
	...
	Возврат СокрЛП(Сред(СтрокаПоиска, 
		СтрДлина(ПараметрыВыполнения.ПолныйКлючСПараметрами) + СтрДлина(ПараметрыВыполнения.Разделитель)));
...
Функция СтрокаПараметровКомандыПоиска(ПараметрыПоиска, Команда, Параметры) Экспорт
	...
	РазделительМеждуКлючемИЗначением = ПараметрыПоиска.РазделительМеждуКлючемИЗначениемПараметраКоманды;
	...
	Для Каждого КлючИЗначение Из Параметры Цикл
		...
		Если ОписаниеПараметра <> Неопределено Тогда
			...
			КлючИЗначениеСтрокой = СтрШаблон("%1%2%3", 
				КлючПараметра, РазделительМеждуКлючемИЗначением, ЗначениеСтрокой);
...
#Область СтандартныеПараметрыКоманд

// Заполнить стандратные параметры команд.
// 
// Параметры:
//  ПараметрыПоиска - см. ПараметрыРасширенногоГлобальногоПоиска
Процедура ЗаполнитьСтандратныеПараметрыКоманд(ПараметрыПоиска)
	
	ЗаполнитьСтандратныеПараметрыКоманд_ТекущаяСтраница(ПараметрыПоиска);
...
// Параметры:
//  ПараметрыПоиска - см. ПараметрыРасширенногоГлобальногоПоиска
Процедура ЗаполнитьСтандратныеПараметрыКоманд_ТекущаяСтраница(ПараметрыПоиска)
...
Процедура ЗаполнитьКомандыПоискаПоФормамРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	//TODO: Если параметры команды заполнены, то выводить действие "Параметры", чтобы можно было их посмотреть и изменить
	...
	ПараметрКоманды = РГП_ГлобальныйПоискКлиентСервер.ОписаниеПараметраКоманды(
		КлючПараметра, "Идентификатор формы", "УникальныйИдентификатор");
...
Процедура ЗаполнитьКомандуНастройкиРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	ПараметрНастройка = РГП_ГлобальныйПоискКлиентСервер.ОписаниеПараметраКоманды(КлючПараметра, "Настройка", "Строка");
...
Процедура ГлобальныйПоискНастройки_ВывестиНастройки(РезультатыПоиска, ПараметрыВыполнения)
	...
	Для Каждого КлючИЗначение Из ОписанияНастроек Цикл
		...
		ПередаваемыеПараметры = Новый Структура(КлючПараметраКомандыРедактораНастройки(), КлючИЗначение.Ключ);
...
Процедура ОбработатьДанныеРезультатаГлобальногоПоиска_ПутьПерехода(ВыполняемоеДействие)
	...
	Если КомандаПоиска = Неопределено Тогда
		ВызватьИсключение "Не найдена команда поиска: " + ВыполняемоеДействие.КомандаПоиска;
	КонецЕсли;
