...
Функция ПараметрыРасширенногоГлобальногоПоиска(ИзКэша = Истина) Экспорт
	...
	ПараметрыСобытия = Новый Массив; // Массив из Неопределено
	ПараметрыСобытия.Добавить(ПараметрыПоиска);
	РГП_ГлобальныйПоискКлиентСервер.ВыполнитьПереопределяемоеСобытие(
		"ПослеЗаполненияПараметровГлобальногоПоиска", Истина, Ложь, ПараметрыСобытия);
