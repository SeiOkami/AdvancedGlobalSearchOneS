...
Функция НоваяКомандаРасширенногоПоиска(ПараметрыВыполнения, Ключ, Родитель = Неопределено, Знач ПорядокРодителя = Неопределено) Экспорт
	...
	//Стандартные команды
	Команда.Вставить("СтандартныеКомандыШапки", Новый Массив); //Массив из Строка
	Команда.Вставить("СтандартныеКомандыПодвала" , Новый Массив); //Массив из Строка
	...
	Иначе
		...
		Команда.СтандартныеКомандыШапки.Добавить(СтандартныеКоманды.ВводСтроки);
		Команда.СтандартныеКомандыШапки.Добавить(СтандартныеКоманды.Обновить);
		
	КонецЕсли;
	
	Команда.СтандартныеКомандыПодвала.Добавить(СтандартныеКоманды.ВывестиКоманды);
...
Процедура ЗаполнитьИнформационныеКомандыРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	КомандаПоиска.Представление = "Текущая версия: " + РГП_ГлобальныйПоискКлиентПовтИсп.ВерсияРасширения();
