...
Асинх Процедура ГлобальныйПоискВычислитьВыражение(ВыполняемоеДействие, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успешно Тогда
		...
		Если ТипЗнч(ДанныеРезультата) = Тип("Строка")
			И ЭтоАдресВременногоХранилища(ДанныеРезультата) Тогда
			Попытка
				Значение = ПолучитьИзВременногоХранилища(ДанныеРезультата); 
			Исключение
				Значение = Результат.Представление;
			КонецПопытки;
		Иначе
			Значение = ДанныеРезультата;
		КонецЕсли;
	Иначе
...
Функция ПараметрыВыполненияКомандыПоиска(ПараметрыПоиска, СтрокаПоиска, Команда, КлючИПараметры) Экспорт
	...
	ПараметрыВыполнения.ДоступенБуферОбмена = ПараметрыПоиска.ДоступенБуферОбмена;
...
Процедура ОбработчикОповещенияСкопироватьВБуфер(ВыполняемоеДействие, КопируемыйТекст) Экспорт
	
	ПоместитьВБуферОбменаТекст(КопируемыйТекст);
...
// * ДоступенБуферОбмена - Булево - В текущей версии платформы доступны мемтоды работы с буфером обмена
...
Функция ПараметрыРасширенногоГлобальногоПоиска(ИзКэша = Истина) Экспорт
	...
	ПараметрыПоиска.Вставить("ДоступенБуферОбмена", ДоступенБуферОбмена());	
	...
	РГП_ГлобальныйПоискКлиентСервер.ВыполнитьПереопределяемоеСобытие(
		"ПослеЗаполненияПараметровГлобальногоПоиска", Истина, Ложь, ПараметрыПоиска);
...
// Исключить стандартные команды из команды поиска.
// 
// Параметры:
//  КомандаПоиска - см. НоваяКомандаРасширенногоПоиска
//  СтандартнаяКоманда1 - Строка - см. РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды
//  СтандартнаяКоманда2 - Строка - см. РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды
//  СтандартнаяКоманда3 - Строка - см. РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды
//  СтандартнаяКоманда4 - Строка - см. РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды
//  СтандартнаяКоманда5 - Строка - см. РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды
// @skip-check method-too-many-params
Процедура ИсключитьСтандартныеКомандыИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда1, 
	СтандартнаяКоманда2 = "", СтандартнаяКоманда3 = "", СтандартнаяКоманда4 = "", СтандартнаяКоманда5 = "") Экспорт
	
	ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда1);
	ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда2);
	ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда3);
	ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда4);
	ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда5);
...
Процедура ИсключитьСтандартнуюКомандуИзКомандыПоиска(КомандаПоиска, СтандартнаяКоманда) Экспорт
	...
	Если ПустаяСтрока(СтандартнаяКоманда) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьКомандыВычисленияВыражения(ПараметрыПоиска, ОбщаяКоманда)
	...
	ИсключитьСтандартныеКомандыИзКомандыПоиска(ОсновнаяКоманда, 
		СтандартныеКоманды.ВводСтроки, СтандартныеКоманды.Обновить, СтандартныеКоманды.Избранное);
...
Процедура ЗаполнитьКомандуПоискаИзбранные(ПараметрыПоиска, ОбщаяКоманда)
	...
	СтандартныеКоманды = РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды();
	ИсключитьСтандартныеКомандыИзКомандыПоиска(КомандаПоиска,
		СтандартныеКоманды.Обновить, СтандартныеКоманды.ВводСтроки, СтандартныеКоманды.Избранное);
...
Процедура ЗаполнитьКомандуНастройкиРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	СтандартныеКоманды = РГП_ГлобальныйПоискКлиентСервер.СтандартныеКоманды();
	ИсключитьСтандартныеКомандыИзКомандыПоиска(Команда,
		СтандартныеКоманды.ВводСтроки, СтандартныеКоманды.Обновить, СтандартныеКоманды.Избранное);
...
Процедура ЗаполнитьИнформационныеКомандыРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	ИсключитьСтандартныеКомандыИзКомандыПоиска(ИнформационнаяКоманда,
		СтандартныеКоманды.ВводСтроки, СтандартныеКоманды.Обновить, СтандартныеКоманды.Избранное);
...
Функция ДоступенБуферОбмена()
	
	Возврат НЕ СредстваБуфераОбмена() = Неопределено;
	
КонецФункции

// Возвращаемое значение:
//  СредстваБуфераОбмена, Неопределено - Средства буфера обмена
Функция СредстваБуфераОбмена()
	
	Попытка
		// BSLLS:ExecuteExternalCodeInCommonModule-off
		БуферОбмена = Вычислить("СредстваБуфераОбмена"); // СредстваБуфераОбмена
		Если БуферОбмена.ИспользованиеДоступно() Тогда
			Возврат БуферОбмена;
		Иначе
			Возврат Неопределено;
		КонецЕсли;
...
// Параметры:
//  Текст - Строка
Процедура ПоместитьВБуферОбменаТекст(Текст)
	
	БуферОбмена = СредстваБуфераОбмена();
	Если БуферОбмена = Неопределено Тогда
		ТекстИсключения = "Средства работы с буфером обмена доступны начиная с версии платформы 8.3.24";
		ТекстИсключения = ФразаИнтерфейса(ТекстИсключения);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	ТекстСкрипта  = "Новый ЭлементБуфераОбмена(СтандартныйФорматДанныхБуфераОбмена.Текст, Текст)";
	ЭлементБуфера = Вычислить(ТекстСкрипта); // см. ЭлементБуфераОбмена
	
	ПомещаемыеДанные = Новый Массив; // Массив из Неопределено
	ПомещаемыеДанные.Добавить(ЭлементБуфера);
	
	БуферОбмена.ПоместитьДанныеАсинх(ПомещаемыеДанные);
