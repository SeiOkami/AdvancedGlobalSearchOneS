...
Процедура ОбработчикСтандартнойКомандыПоиска_Настройки(СтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	ВыполняемоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеПереходаНаКоманду(
		"\Настройки\", ПараметрыКоманды);
	
	ПредставлениеКоманды = РГП_ГлобальныйПоискКлиентСервер.СтрокаЗаголовком("Настройки");
	ЭлементРезультата = РГП_ГлобальныйПоискКлиентСервер.ЭлементРезультатаГлобальногоПоиска(
...
Процедура ОбработчикСтандартнойКомандыПоиска_Избранное(СтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	ПараметрыПоиска   = ПараметрыРасширенногоГлобальногоПоиска();
	ЭлементИзбранного = ПараметрыПоиска.Избранное.Получить(ВРег(ПараметрыВыполнения.ПолнаяСтрокаПоиска));
	
	Если ЭлементИзбранного = Неопределено Тогда
		ИмяМетода = "ДобавитьВИзбранное";
	...
	ОписаниеОповещения = РГП_ГлобальныйПоискКлиентСервер.ОписаниеОповещения(
		ИмяМетода, ИмяТекущегоМодуля(), ПараметрыВыполнения);
	ДействиеИзбранного = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеОписанияОповещения(ОписаниеОповещения);
	
	ДействиеОбновления = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеОбновитьРезультаты(ПараметрыВыполнения);
	
	НаборДействий = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеНабораДействий(
		ДействиеИзбранного, ДействиеОбновления);
	
	Представление = РГП_ГлобальныйПоискКлиентСервер.СтрокаЗаголовком(Представление);
	ЭлементРезультата = РГП_ГлобальныйПоискКлиентСервер.ЭлементРезультатаГлобальногоПоиска(
		НаборДействий, Представление, БиблиотекаКартинок.Избранное);
...
Процедура ГлобальныйПоискИзбранное(ПолнаяСтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	ПараметрыПоиска = ПараметрыРасширенногоГлобальногоПоиска();
	Для Каждого КлючИЗначение Из ПараметрыПоиска.Избранное Цикл
		
		ЭлементИзбранного = КлючИЗначение.Значение;
		
		ОписаниеОповещения = РГП_ГлобальныйПоискКлиентСервер.ОписаниеОповещения(
			"ОбработчикВыполнитьГлобальныйПоискПоСтроке", ИмяТекущегоМодуля(), ЭлементИзбранного.СтрокаПоиска);
		ВыполняемоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеОписанияОповещения(ОписаниеОповещения);
		
		РГП_ГлобальныйПоискКлиентСервер.ДобавитьЭлементРезультатаГлобальногоПоиска(
			РезультатыПоиска, ВыполняемоеДействие, ЭлементИзбранного.Представление);
...
//  ВыполняемоеДействие - см. РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействие
//  ПараметрыВыполнения - см. РГП_ГлобальныйПоискКлиентСервер.ШаблонПараметровВыполненияКомандыПоиска
Процедура ДобавитьВИзбранное(ВыполняемоеДействие, ПараметрыВыполнения) Экспорт
	
	ЭлементИзбранного = Новый ЭлементИзбранногоРаботыПользователя;
	ЭлементИзбранного.НавигационнаяСсылка = НавигационнаяСсылкаСтрокиПоиска(ПараметрыВыполнения.ПолнаяСтрокаПоиска);
	ЭлементИзбранного.Представление = ПредставлениеКомандыДляИзбранного(ПараметрыВыполнения);
	
	МенеджерИзбранного = Новый ИзбранноеРаботыПользователя();
	МенеджерИзбранного.Добавить(ЭлементИзбранного);
	
	ОписаниеЭлемента = РГП_ГлобальныйПоискКлиентСервер.ЭлементИзбранного();
	ЗаполнитьЗначенияСвойств(ОписаниеЭлемента, ЭлементИзбранного);
	ОписаниеЭлемента.СтрокаПоиска = ПараметрыВыполнения.ПолнаяСтрокаПоиска;
	
	ПараметрыПоиска = ПараметрыРасширенногоГлобальногоПоиска();
	ПараметрыПоиска.Избранное.Вставить(ВРег(ПараметрыВыполнения.ПолнаяСтрокаПоиска), ОписаниеЭлемента);
...
//  ВыполняемоеДействие - см. РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействие
//  ПараметрыВыполнения - см. РГП_ГлобальныйПоискКлиентСервер.ШаблонПараметровВыполненияКомандыПоиска
Процедура УдалитьИзИзбранного(ВыполняемоеДействие, ПараметрыВыполнения) Экспорт
	
	НавигационнаяСсылка = НавигационнаяСсылкаСтрокиПоиска(ПараметрыВыполнения.ПолнаяСтрокаПоиска);
	
	МенеджерИзбранного = Новый ИзбранноеРаботыПользователя();
	Для Каждого Элемент Из МенеджерИзбранного Цикл
		Если ВРег(Элемент.НавигационнаяСсылка) = ВРег(НавигационнаяСсылка) Тогда
			Индекс = МенеджерИзбранного.Индекс(Элемент); // Число
			МенеджерИзбранного.Удалить(Индекс);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	ПараметрыПоиска = ПараметрыРасширенногоГлобальногоПоиска();
	ПараметрыПоиска.Избранное.Удалить(ВРег(ПараметрыВыполнения.ПолнаяСтрокаПоиска));
...
Функция НавигационнаяСсылкаСтрокиПоиска(СтрокаПоиска) Экспорт
	...
	КлючНавигационнойСсылки = РГП_ГлобальныйПоискКлиентСервер.КлючНавигационнойСсылки();
...
Функция ПредставлениеКомандыДляИзбранного(ПараметрыВыполнения) Экспорт
	...
	Возврат СтрШаблон("%1 (%2)", ПараметрыВыполнения.Представление, ПараметрыВыполнения.СтрокаПоиска);
...
// * Избранное - см. РГП_ГлобальныйПоискКлиентСервер.КоллекцияЭлементовИзбранного
...
Функция ПараметрыРасширенногоГлобальногоПоиска(ИзКэша = Истина) Экспорт
	...
	ПараметрыПоиска.Вставить("Избранное", РГП_ГлобальныйПоискКлиентСервер.КоллекцияЭлементовИзбранного());
	...
	ЗаполнитьИзбранныеКомандыПараметровПоиска(ПараметрыПоиска);
...
Процедура ЗаполнитьКомандуПоискаИзбранные(ПараметрыПоиска, ОбщаяКоманда)
	...
	КомандаПоиска = НоваяКомандаРасширенногоПоиска(ПараметрыПоиска, "Избранное", ОбщаяКоманда);
	КомандаПоиска.Представление = "Избранное";
	КомандаПоиска.Описание = "Избранные команды поиска";
...
Процедура ЗаполнитьКомандыПоискаМетаданныхРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	ТекущаяНастройка.Представление = "Выводить варианты отчетов";
	ТекущаяНастройка.Пояснение = "Стоит ли выводить варианты отчетов и искать по ним?
	|	(может замедлять поиск)";
	ТекущаяНастройка.Тип = Новый ОписаниеТипов("Булево");
	ТекущаяНастройка.ФорматнаяСтрока = "БЛ='Выводить'; БИ=Не выводить;";
...
Процедура ЗаполнитьКомандуНастройкиРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	ПараметрНастройка = РГП_ГлобальныйПоискКлиентСервер.ОписаниеПараметраКоманды(КлючПараметра, "Команда", "Строка");
	РГП_ГлобальныйПоискКлиентСервер.ДобавитьДоступныйПараметрКоманды(Команда.Параметры, ПараметрНастройка);
...
Процедура ГлобальныйПоискНастройки_ВывестиКоманды(РезультатыПоиска, ПараметрыВыполнения)
	...
	Для Каждого КлючИЗначение Из ПараметрыПоиска.ПолныеКлючиКоманд Цикл
		...
		Если ЗначениеЗаполнено(КомандаПоиска.ОписанияНастроек) Тогда
			...
			Представление = СтрШаблон("Команда: %1", КомандаПоиска.Представление);
			Представление = РГП_ГлобальныйПоискКлиентСервер.СтрокаЗаголовком(Представление);
...
Процедура ГлобальныйПоискНастройки_КомандыОчистки(РезультатыПоиска, ПараметрыВыполнения, КомандаИзПараметров)
	...
	ГлобальныйПоискНастройки_ДобавитьКомандуОчистки(
		РезультатыПоиска, ПараметрыВыполнения, ПараметрыОчистки, "Сбросить настройки");
...
// Заполнить избранные команды параметров поиска.
// 
// Параметры:
//  ПараметрыПоиска - см. ПараметрыРасширенногоГлобальногоПоиска
Процедура ЗаполнитьИзбранныеКомандыПараметровПоиска(ПараметрыПоиска)
	
	Менеджер = Новый ИзбранноеРаботыПользователя;
	Для Каждого Элемент Из Менеджер Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
			ЭлементИзбранного = РГП_ГлобальныйПоискКлиентСервер.ЭлементИзбранного();
			ЭлементИзбранного.СтрокаПоиска = СтрокаПоиска;
			ЗаполнитьЗначенияСвойств(ЭлементИзбранного, Элемент);
			ПараметрыПоиска.Избранное.Вставить(ВРег(СтрокаПоиска), ЭлементИзбранного);
		КонецЕсли;
