...
Процедура ОбработчикСтандартнойКомандыПоиска_Параметры(СтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Если НЕ ПараметрыВыполнения.ПользовательскиеНастройки.ГруппироватьСтандартныеКоманды Тогда
		ЧастиПредставления.Вставить(0, РГП_ГлобальныйПоискКлиентСервер.СтрокаЖирным("Параметры:"));
		ЭлементРезультата = РГП_ГлобальныйПоискКлиентСервер.ЭлементРезультатаГлобальногоПоиска(
			Неопределено, "", БиблиотекаКартинок.Параметры);
		РезультатыПоиска.Добавить(ЭлементРезультата);
	КонецЕсли;
...
Функция НоваяКомандаРасширенногоПоиска(ПараметрыВыполнения, Ключ, 
	...
	Иначе
		...
		ИспользуемыеСтандартныеКоманды.Добавить(СтандартныеКоманды.Параметры);
...
Процедура ГлобальныйПоискПоОткрытымФормам_ВывестиОткрытыеФормы(ОбщиеПараметры)
	...
	МаксимумСтрокРезультата = ОбщиеПараметры.ПараметрыВыполнения.ПользовательскиеНастройки.МаксимумСтрокРезультата;
	...
	Для Каждого Окно Из ОткрытыеОкна Цикл
		...
		Для Каждого Форма Из Окно.Содержимое Цикл
			...
			Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения")
				И ОбщиеПараметры.ВыведеноФорм < МаксимумСтрокРезультата Тогда
				
				ГлобальныйПоискПоОткрытымФормам_ВывестиОткрытуюФорму(ОбщиеПараметры, Форма, Окно);
				
				ОбщиеПараметры.ВыведеноФорм = ОбщиеПараметры.ВыведеноФорм + 1;
...
Процедура ГлобальныйПоискПоЭлементамФормы_ДобавитьРезультаты(ОбщиеПараметры, РезультатыПоиска)
	...
	Для Каждого ЭлементКоллекции Из КоллекцияЭлементов Цикл
		...
		Если РезультатыПоиска.Количество() = ПараметрыВыполнения.ПользовательскиеНастройки.МаксимумСтрокРезультата Тогда
			Прервать;
		КонецЕсли;
