...
Процедура ПриГлобальномПоиске(ИзначальнаяСтрокаПоиска, ПланПоиска) Экспорт
	...
	Для Каждого КлючИЗначение Из ПараметрыПоиска.Команды Цикл
		...
		ПараметрыВыполнения = ПараметрыВыполненияКомандыПоиска(ПараметрыПоиска, ИзначальнаяСтрокаПоиска, ТекущаяКоманда);
...
Процедура ОбработкаПереходаПоНавигационнойСсылке(
	...
	КлючГлобальногоПоиска = "ГлобальныйПоиск";
	
	Если СтрНачинаетсяС(НавигационнаяСсылка, КлючГлобальногоПоиска) Тогда
		
		СтандартнаяОбработка = Ложь;
		
		СтрокаПоиска = Сред(НавигационнаяСсылка, СтрДлина(КлючГлобальногоПоиска) + 1);
...
Процедура ДобавитьОбщийЭлементРезультатаСтандартныхКоманд(РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Представление = Новый ФорматированнаяСтрока(Представление, 
		РГП_ГлобальныйПоискКлиентСервер.ЖирныйШрифт(), WebЦвета.ПолночноСиний);
	
	РГП_ГлобальныйПоискКлиентСервер.ДобавитьЭлементРезультатаГлобальногоПоиска(
		РезультатыПоиска, ПустоеДействие, Представление, БиблиотекаКартинок.КомандаМенюФункций);
...
Процедура ГлобальныйПоискНастройки(ПолнаяСтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ПараметрыКоманды) Тогда
	
		ГлобальныйПоискНастройки_ВывестиЗначения(РезультатыПоиска, ПараметрыВыполнения);
	
	Иначе
				
		ГлобальныйПоискНастройки_ВывестиНастройки(РезультатыПоиска, ПараметрыВыполнения);
...
Процедура ОбработчикВыбораНовогоЗначенияПользовательскойНастройки(ВыполняемоеДействие, Параметры) Экспорт
	...
	ИзменитьОбщуюПользовательскуюНастройку(Параметры.НовоеЗначение, Параметры.КлючНастройки);
	
КонецПроцедуры

// Изменить общую пользовательскую настройку.
// 
// Параметры:
//  НовоеЗначение - Произвольный
//  КлючНастройки - Строка
Процедура ИзменитьОбщуюПользовательскуюНастройку(НовоеЗначение, КлючНастройки) Экспорт
	
	ПараметрыПоиска = ПараметрыРасширенногоГлобальногоПоиска();
	ТекущиеНастройки = ПараметрыПоиска.ПользовательскиеНастройки.ОбщиеНастройки;
	ОписаниеНастройки = РГП_ГлобальныйПоискКлиентСервер.ОписаниеПользовательскойНастройки(
		ПараметрыПоиска.ОписанияПользовательскихНастроек, КлючНастройки);
	
	РГП_ГлобальныйПоискКлиентСервер.ПривестиЗначениеКПользовательскойНастройке(НовоеЗначение, ОписаниеНастройки);
	
	ТекущееЗначение = Неопределено;
	ЗначениеПреставлениеОбщейПользовательскойНастройки(КлючНастройки, ТекущееЗначение, "", ПараметрыПоиска);
	Если ТекущееЗначение = НовоеЗначение Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеНастройки[КлючНастройки] = НовоеЗначение;
...
Функция ПараметрыВыполненияКомандыПоиска(ПараметрыПоиска, СтрокаПоиска, Команда) Экспорт
...
// * РазделительМеждуКлючемИЗначениемПараметраКоманды - Строка - Символ-разделитель между ключем команды и его значением
...
Функция ПараметрыРасширенногоГлобальногоПоиска(ИзКэша = Истина) Экспорт
	...
	ПараметрыПоиска.Вставить("РазделительМеждуКлючемИЗначениемПараметраКоманды", "=");
...
// Выполняемое действие перехода назад.
// 
// Параметры:
//  ПараметрыВыполнения - см. ПараметрыВыполненияКомандыПоиска
// 
// Возвращаемое значение:
//  см. РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеПереходаНаКоманду
Функция ВыполняемоеДействиеПереходаНазад(ПараметрыВыполнения) Экспорт
	...
	Возврат РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеПереходаНаКоманду(ВременныеПараметры);
...
#Область ВспомогательныеСлужебные
...
Функция КлючКомандыИПараметрыИзЧастиСтрокиПоиска(ЧастьСтрокиПоиска) Экспорт
	...
	Иначе
		...
		СтрокаПараметров = Сред(ЧастьСтрокиПоиска, КоординатаПараметров + СтрДлина(РазделительКлючаИПараметров));
		
		РазделительПараметров = ПараметрыПоиска.РазделительМеждуПараметрамиКоманды;
		РазделительЗначений = ПараметрыПоиска.РазделительМеждуКлючемИЗначениемПараметраКоманды;
...
Процедура ЗаполнитьКомандуНастройкиРасширенногоГлобальногоПоиска(ПараметрыПоиска, ОбщаяКоманда)
	...
	Команда = НоваяКомандаРасширенногоПоиска(ПараметрыПоиска, "Настройки", ОбщаяКоманда);
	Команда.ВозможныПараметры = Истина;
...
//  РезультатыПоиска - РезультатГлобальногоПоиска - Выходной параметр. Содержит результаты поиска.
//  ПараметрыВыполнения - см. РГП_ГлобальныйПоискКлиентСервер.ШаблонПараметровВыполненияКомандыПоиска
Процедура ГлобальныйПоискНастройки_ВывестиНастройки(РезультатыПоиска, ПараметрыВыполнения)
	...
	Для Каждого КлючИЗначение Из ОписанияНастроек Цикл
		...
		ЗначениеПреставлениеОбщейПользовательскойНастройки(КлючИЗначение.Ключ, 
			ТекущееЗначение, ПредставлениеЗначения, ПараметрыПоиска);
		
		ПередаваемыеПараметры = Новый Массив; // Массив из Строка
		ПередаваемыеПараметры.Добавить(ОписаниеНастройки.Ключ);
		ВыполняемоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеПереходаНаКоманду(
			ПараметрыВыполнения, , ПередаваемыеПараметры);
		
		ПредставлениеНастройки = Новый ФорматированнаяСтрока(
			РГП_ГлобальныйПоискКлиентСервер.СтрокаЖирным(ОписаниеНастройки.Представление),
			":	", ПредставлениеЗначения);
...
//  РезультатыПоиска - РезультатГлобальногоПоиска - Выходной параметр. Содержит результаты поиска.
//  ПараметрыВыполнения - см. РГП_ГлобальныйПоискКлиентСервер.ШаблонПараметровВыполненияКомандыПоиска
Процедура ГлобальныйПоискНастройки_ВывестиЗначения(РезультатыПоиска, ПараметрыВыполнения)
	
	ПараметрыПоиска = ПараметрыРасширенногоГлобальногоПоиска();
	ОписанияНастроек = ПараметрыПоиска.ОписанияПользовательскихНастроек;
	
	КлючНастройки = ПараметрыВыполнения.ПараметрыКоманды[0];
	
	ОписаниеНастройки = РГП_ГлобальныйПоискКлиентСервер.ОписаниеПользовательскойНастройки(
		ОписанияНастроек, КлючНастройки);
	
	ПояснениеНастройки = Новый ФорматированнаяСтрока(ОписаниеНастройки.Пояснение, , WebЦвета.ТемноСиний);
	ПустоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеПустое();
	РГП_ГлобальныйПоискКлиентСервер.ДобавитьЭлементРезультатаГлобальногоПоиска(
	...
	ТекущееЗначение = Неопределено;
	ПредставлениеЗначения = "";
	ЗначениеПреставлениеОбщейПользовательскойНастройки(КлючНастройки, 
		ТекущееЗначение, ПредставлениеЗначения, ПараметрыПоиска);
	...
	ГлобальныйПоискНастройки_ВывестиЗначения_ДоступныеЗначения(ПараметрыВыполнения, 
		РезультатыПоиска, ОписаниеНастройки, ДоступныеЗначения, ТекущееЗначение, ПредставлениеЗначения);
	
	Если ПоказатьВводЗначения Тогда
		
		ГлобальныйПоискНастройки_ВывестиЗначения_ЭлементВводЗначения(ПараметрыВыполнения, 
			РезультатыПоиска, ОписаниеНастройки, ТекущееЗначение, ПредставлениеЗначения);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ГлобальныйПоискНастройки_ВывестиЗначения_ДоступныеЗначения(ПараметрыВыполненияКоманды, 
	РезультатыПоиска, ОписаниеНастройки, ДоступныеЗначения, ТекущееЗначение, ПредставлениеЗначения)
	...
	Для Каждого ДоступноеЗначение Из ДоступныеЗначения Цикл
		...
		Если ДоступноеЗначение.Значение = ТекущееЗначение Тогда
			ТекущееПредставление = РГП_ГлобальныйПоискКлиентСервер.СтрокаЖирным(ТекущееПредставление);
		КонецЕсли;
		...
		ВыполняемоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеОписанияОповещения(
			ОписаниеОповещения);
		ВыполняемоеДействие.ПутьПерехода = ПараметрыВыполненияКоманды.ПолныйКлюч;
...
Процедура ГлобальныйПоискНастройки_ВывестиЗначения_ЭлементВводЗначения(ПараметрыВыполненияКоманды, 
	РезультатыПоиска, ОписаниеНастройки, ТекущееЗначение, ПредставлениеЗначения)
	
	КартинкаПоУмолчанию = БиблиотекаКартинок.Реквизит;
	
	ТекущееПредставление = "Текущее значение: " + ПредставлениеЗначения;
	ОписаниеОповещения = РГП_ГлобальныйПоискКлиентСервер.ОписаниеОповещения(
		"ИзменитьОбщуюПользовательскуюНастройку", ИмяТекущегоМодуля(), ОписаниеНастройки.Ключ);
	...
	ВыполняемоеДействие = РГП_ГлобальныйПоискКлиентСервер.ВыполняемоеДействиеВводаЗначения(ПараметрыДействия);
	ВыполняемоеДействие.ПутьПерехода = ПараметрыВыполненияКоманды.ПолныйКлюч;
...
// Значение общей пользовательской настройки.
// 
// Параметры:
//  КлючНастройки - Строка
...
//  ПараметрыПоиска - см. ПараметрыРасширенногоГлобальногоПоиска
// 
Процедура ЗначениеПреставлениеОбщейПользовательскойНастройки(
	Знач КлючНастройки, Значение, Представление = "", Знач ПараметрыПоиска = Неопределено)
	...
	Если ПараметрыПоиска.ПользовательскиеНастройки.ОбщиеНастройки.Свойство(
		КлючНастройки, ПользовательскоеЗначение) Тогда
