...
Асинх Процедура ГлобальныйПоискВычислитьВыражение(ВыполняемоеДействие, ДополнительныеПараметры) Экспорт
	...
	ТекстВыражения = Ждать ВвестиСтрокуАсинх(ДополнительныеПараметры.НачальноеВыражение, "Введите выражение", , Истина);
...
Функция НоваяКомандаРасширенногоПоиска(ПараметрыВыполнения, Ключ, Родитель = Неопределено, Знач ПорядокРодителя = Неопределено) Экспорт
	...
	Команда.Вставить("ЛимитСтрокРезультата", 3);
...
Процедура ЗаполнитьКомандыВычисленияВыражения(ПараметрыПоиска, ОбщаяКоманда)
	...
	КомандаЭлементы.Представление = "&НаКлиенте";
	...
	КомандаЭлементы.Представление = "&НаСервере";
	КомандаЭлементы.Картинка = БиблиотекаКартинок.СформироватьОтчет;
