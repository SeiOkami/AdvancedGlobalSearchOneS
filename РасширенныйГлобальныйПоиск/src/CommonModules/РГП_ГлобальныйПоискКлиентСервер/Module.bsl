...
Функция ВыполняемоеДействиеВводНомераСтраницы(ПараметрыВыполнения, ТекущаяСтраница) Экспорт
	...
	ПараметрыВвода = ПараметрыВыполняемогоДействияВводаЗначения(ОписаниеОповещения, 
		ТекущаяСтраница, ФразаИнтерфейса("Введите страницу"), ОписаниеТипаПараметраКоманды_Страница());
...
// Параметры:
//  КопируемыйТекст - Строка
// 
// Возвращаемое значение:
//  см. ВыполняемоеДействие
Функция ВыполняемоеДействиеСкопироватьВБуфер(КопируемыйТекст) Экспорт
	
	ОписаниеОповещения  = ОписаниеОповещения("ОбработчикОповещенияСкопироватьВБуфер", 
		"РГП_ГлобальныйПоискКлиент", КопируемыйТекст);
	Возврат ВыполняемоеДействиеОписанияОповещения(ОписаниеОповещения);
...
// * ДоступенБуферОбмена - Булево - Доступены методы работы с буфером обмена
...
Функция ШаблонПараметровВыполненияКомандыПоиска() Экспорт
	...
	Результат.Вставить("ДоступенБуферОбмена", Ложь);
...
Процедура ДобавитьЭлементРезультатаСтандартнойКоманды_Обновить(РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Представление = ФразаИнтерфейса("Обновить");
	Представление = СтрокаЗаголовком(Представление);
	
	ВыполняемоеДействие = ВыполняемоеДействиеОбновитьРезультаты(ПараметрыВыполнения);
	ЭлементРезультата = ЭлементРезультатаГлобальногоПоиска(
		ВыполняемоеДействие, Представление, БиблиотекаКартинок.Обновить);
...
Процедура ДобавитьЭлементРезультатаСтандартнойКоманды_Назад(РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Представление = ФразаИнтерфейса("Назад");
	Представление = СтрокаЗаголовком(Представление);
...
Процедура ДобавитьЭлементРезультатаСтандартнойКоманды_Страница(
	...
	ФразаИз = ФразаИнтерфейса("из");
	ФразаСтраница = ФразаИнтерфейса("Страница");
	ФразаВыведено = ФразаИнтерфейса("Выведено");
	
	Представление = СтрШаблон(
		"%1 %2 %3 %4	|	%5 %6 %7 %8",
		ФразаСтраница, ТекущаяСтраница, ФразаИз, ПараметрыСтраниц.ВсегоСтраниц,
		ФразаВыведено, ПараметрыСтраниц.ОсталосьВывести, ФразаИз, ПараметрыСтраниц.ВсегоРезультатов);
...
Функция ЭлементРезультатаКомандаВводаСтроки(ПараметрыВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(НачальнаяСтрока) 
		...
		Представление = ФразаИнтерфейса("Строка поиска");
		Представление = СтрШаблон("%1: %2", Представление, НачальнаяСтрока);
	Иначе
		Представление = ФразаИнтерфейса("Поиск");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НачальнаяСтрока) 
		...
		ФразаОчистить = ФразаИнтерфейса("Очистить");
		ДействиеОчистить = ВыполняемоеДействиеПереходаНаКоманду(
			ПараметрыВыполнения.ПолныйКлюч, ПараметрыВыполнения.ПараметрыКоманды);
		ЭлементРезультата.Действия.Добавить(ДействиеОчистить, ФразаОчистить, БиблиотекаКартинок.Очистить);
...
Процедура ОбработатьОшибку(ИнформацияОбОшибке) Экспорт
	...
	ВывестиСообщение(КраткоеПредставление);
	
	ПодробноеПредставление = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	РГП_ГлобальныйПоискВызовСервера.ДобавитьЗаписьЖР(ПодробноеПредставление);
...
Функция ОписанияДоступныхПользовательскихНастроек() Экспорт
	...
	ФразаГруппировать = ФразаИнтерфейса("Группировать");
	ФразаНеГруппировать = ФразаИнтерфейса("Не группировать");
	ФорматнаяСтрока = СтрШаблон("БЛ='%1'; БИ=%2;", ФразаНеГруппировать, ФразаГруппировать);
	
	ТекущаяНастройка = НовыйОписаниеПользовательскойНастройки();
	ТекущаяНастройка.Ключ = "ГруппироватьСтандартныеКоманды";
	ТекущаяНастройка.Представление = ФразаИнтерфейса("Группировать стандартные команды");
	ТекущаяНастройка.Пояснение = ФразаИнтерфейса("Стоит ли группировать стандартные команды в одном элементе");
	ТекущаяНастройка.Тип = Новый ОписаниеТипов("Булево");
	ТекущаяНастройка.ДоступноАвто = Истина;
	ТекущаяНастройка.ФорматнаяСтрока = ФорматнаяСтрока;
	Результат.Вставить(ТекущаяНастройка.Ключ, ТекущаяНастройка);
	
	ТекущаяНастройка = НовыйОписаниеПользовательскойНастройки();
	ТекущаяНастройка.Ключ = "МаксимумСтрокРезультата";
	ТекущаяНастройка.Представление = ФразаИнтерфейса("Максимум строк результата");
	ТекущаяНастройка.Пояснение = ФразаИнтерфейса("Какое максимальное число строк результата выводить");
...
Функция ОписаниеПользовательскойНастройки(ОписанияНастроек, КлючНастройки, ВызыватьИсключение = Истина) Экспорт
	...
	Если ОписаниеНастройки = Неопределено И ВызыватьИсключение Тогда
		ТекстИсключение = ФразаИнтерфейса("Не найдено описание пользовательской настройки");
		ТекстИсключение = СтрШаблон("%1: %2", ТекстИсключение, КлючНастройки);
...
Функция ОписаниеПараметраКоманды_Страница() Экспорт
	...
	Представление = ФразаИнтерфейса("Страница");
	ОписаниеПараметра = ОписаниеПараметраКоманды(КлючПараметра, Представление, ОписаниеТипаПараметраКоманды_Страница());
...
Функция ОписаниеПараметраКоманды_Тип() Экспорт
	...
	ОписаниеПараметра = ОписаниеПараметраКоманды(КлючПараметра, ФразаИнтерфейса("Тип"), "Строка");
...
Функция КлючПараметраКоманды_Настройки_Команда() Экспорт
	...
	Возврат ФразаИнтерфейса("Команда");
...
Функция ШрифтЗаголовковМеню() Экспорт
	...
	Возврат ШрифтСтиля("РГП_ШрифтМеню");
...
Функция МелкийШрифтТекста() Экспорт
	...
	ИмяШрифта = ФразаИнтерфейса("МелкийШрифтТекста");
	Возврат ШрифтСтиля(ИмяШрифта);
	
КонецФункции

// Возвращаемое значение:
//  Шрифт
Функция КрупныйШрифтТекста() Экспорт
	
	ИмяШрифта = ФразаИнтерфейса("КрупныйШрифтТекста");
	Возврат ШрифтСтиля(ИмяШрифта);
...
Функция ЦветФонаВыделенияПоля() Экспорт
	...
	ИмяЦвета = ФразаИнтерфейса("ЦветФонаВыделенияПоля");
	Возврат ЦветСтиля(ИмяЦвета);
...
Функция ЦветВажного() Экспорт
	...
	ИмяЦвета = ФразаИнтерфейса("ЦветВажного");
	Возврат ЦветСтиля(ИмяЦвета);
...
Функция ЦветТекстаШапкиТаблицы() Экспорт
	...
	ИмяЦвета = ФразаИнтерфейса("ЦветТекстаШапкиТаблицы");
	Возврат ЦветСтиля(ИмяЦвета);
...
Функция МассивИменРолейИзПараметра(ИменаРолей) Экспорт
	...
	Иначе
		ТекстИсключения = ФразаИнтерфейса("Передан параметр некорректного типа");
		ТекстИсключения = СтрШаблон("%1 %2", ТекстИсключения, ТипЗнч(ИменаРолей));
...
Функция ВычислитьВыражение(ТекстВыражения, НаСервере = Ложь) Экспорт
	...
	Иначе
		...
		Попытка
			РГП_ГлобальныйПоискВызовСервера.ПроверитьЭтоПолноправныйПользователь();
		...
		Исключение
			...
			ЛишнееНачало = ФразаИнтерфейса("Неизвестный модуль");
			ЛишнееНачало = СтрШаблон("{<%1>(1,1)}: ", ЛишнееНачало);
...
Функция ЭтоИмяМетода(ИмяМетода, ВызыватьИсключение = Истина) Экспорт
	...
	Исключение
		Если ВызыватьИсключение Тогда
			ТекстИсключения = ФразаИнтерфейса("Значение не соответствует имени метода");
			ТекстИсключения = СтрШаблон("%1: %2", ТекстИсключения, ИмяМетода);
			ВызватьИсключение ТекстИсключения;
		Иначе
			Возврат Ложь;
		КонецЕсли;
...
Процедура ВызватьИсключениеНедостаточноПрав() Экспорт
	
	ВызватьИсключение ФразаИнтерфейса("Недостаточно прав!");
...
Процедура ВыполнитьПереопределяемоеСобытие(ИмяСобытия, Клиент, Сервер, Параметр = Неопределено) Экспорт
	
	ОбщиеМодули = ПереопределяемыеОбщиеМодули(Клиент, Сервер);
	Для Каждого ИмяМодуля Из ОбщиеМодули Цикл
		#Если Сервер Тогда
			Параметры = Новый Массив; // Массив из Неопределено
			Параметры.Добавить(Параметр);
			РГП_ГлобальныйПоискСервер.ВыполнитьМетод(ИмяМодуля, ИмяСобытия, Ложь, Параметры);
		#Иначе
			ПроверитьСоответствуетПравилуИмениКода(ИмяСобытия);
			ПроверитьСоответствуетПравилуИмениКода(ИмяМодуля);
			Модуль = Вычислить(ИмяМодуля); // BSLLS:ExecuteExternalCodeInCommonModule-off
			ОписаниеОповещения = Новый ОписаниеОповещения(ИмяСобытия, Модуль);
			ВыполнитьОбработкуОповещения(ОписаниеОповещения, Параметр);
		#КонецЕсли
	КонецЦикла;
...
Функция ПереопределяемыеОбщиеМодули(Клиент, Сервер) Экспорт
	...
	Возврат РГП_ГлобальныйПоискКлиентСерверПовтИсп.ПереопределяемыеОбщиеМодули(Клиент, Сервер);
...
Функция КлючНавигационнойСсылки() Экспорт
	...
	Возврат ФразаИнтерфейса("ГлобальныйПоиск");
...
// Возвращаемое значение:
//  Булево
Функция СоответствуетПравилуИмениКода(Строка) Экспорт
	
	ВременнаяСтруктура = Новый Структура;
	Попытка
		ВременнаяСтруктура.Вставить(Строка);
		Возврат Истина;
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
// Проверить соответствует правилу имени кода.
// 
// Параметры:
//  Строка - Строка
Процедура ПроверитьСоответствуетПравилуИмениКода(Строка) Экспорт
	
	Если НЕ СоответствуетПравилуИмениКода(Строка) Тогда
		ТекстИсключения = ФразаИнтерфейса("Некорректное значение параметра");
		ТекстИсключения = СтрШаблон("%1: %2", ТекстИсключения, Строка);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
