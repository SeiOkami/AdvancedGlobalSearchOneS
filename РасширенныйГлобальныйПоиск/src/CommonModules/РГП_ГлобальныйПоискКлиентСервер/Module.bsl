...
Функция ПараметрыСтраницРезультатаКоманды(ПараметрыВыполнения, ВсегоРезультатов) Экспорт
	...
	ЧислоСтраницСХвостом = ВсегоРезультатов / Макс(МаксимумСтрок, 1);
	ВсегоСтраниц = Цел(ЧислоСтраницСХвостом);
	Если ЧислоСтраницСХвостом > ВсегоСтраниц Тогда
	...
	ОсталосьВывести = Макс(ОсталосьВывести, 0);
...
Процедура ДобавитьЭлементРезультатаСтандартнойКоманды_Страница(
	...
	Если ПараметрыСтраниц.ВсегоСтраниц < 2 Тогда
		Возврат;
	КонецЕсли;
	
	Представление = Новый Массив; // Массив из Строка
	Если ЗначениеЗаполнено(ПараметрыСтраниц.ВсегоРезультатов) Тогда
		Представление.Добавить(СтрШаблон("Найдено %1 результатов
		|
		|", ПараметрыСтраниц.ВсегоРезультатов));
	КонецЕсли;
	
	ТекущаяСтраница = ПараметрыСтраниц.ТекущаяСтраница;
	
	Представление.Добавить(СтрШаблон("Страница %1 из %2", ТекущаяСтраница, ПараметрыСтраниц.ВсегоСтраниц));
	...
	Если ТекущаяСтраница > 1 Тогда
		...
		ЭлементРезультата.Действия.Добавить(ДействиеПерваяСтраница, СтрокаЗаголовком("<<"));//, БиблиотекаКартинок.ПерейтиКНачалу);
		
		ДействиеПредыдущаяСтраница = ВыполняемоеДействиеПереходаНаСтраницу(
			ПараметрыВыполнения, ТекущаяСтраница - 1);
		ЭлементРезультата.Действия.Добавить(ДействиеПредыдущаяСтраница, СтрокаЗаголовком("<"));//, БиблиотекаКартинок.ПерейтиНазад);
		
	КонецЕсли;
	
	ПредставлениеСтраницы = СтрокаЗаголовком(СтрШаблон(" --%1-- ", ТекущаяСтраница));
	ДействиеВвестиСтраницу = ВыполняемоеДействиеОбновитьРезультаты(ПараметрыВыполнения);
	ЭлементРезультата.Действия.Добавить(ДействиеВвестиСтраницу, ПредставлениеСтраницы);
	
	ОсталосьСтраниц = (ПараметрыСтраниц.ВсегоСтраниц - ПараметрыСтраниц.ТекущаяСтраница);
	Если ОсталосьСтраниц > 0 Тогда
		
		ДействиеСледующаяСтраница = ВыполняемоеДействиеПереходаНаСтраницу(
			ПараметрыВыполнения, ТекущаяСтраница + 1);
		ЭлементРезультата.Действия.Добавить(ДействиеСледующаяСтраница , СтрокаЗаголовком(">"));//, БиблиотекаКартинок.ПерейтиВперед);

		ДействиеПоследняяСтраница  = ВыполняемоеДействиеПереходаНаСтраницу(
			ПараметрыВыполнения, ПараметрыСтраниц.ВсегоСтраниц);
		ЭлементРезультата.Действия.Добавить(ДействиеПоследняяСтраница , СтрокаЗаголовком(">>"));//, БиблиотекаКартинок.ПерейтиККонцу);
...
Функция ОписаниеПараметраКоманды_Страница() Экспорт
	...
	Возврат ОписаниеПараметраКоманды(КлючПараметра, "Страница", ОписаниеТипаПараметраКоманды_Страница());
...
Функция ЗначениеПараметраКоманды_Страница(ПараметрыВыполнения) Экспорт
	...
	ТипПараметра = ОписаниеТипаПараметраКоманды_Страница();
	ТекущаяСтраница = ТипПараметра.ПривестиЗначение(ТекущаяСтраница);
