...
Процедура ГлобальныйПоискМетаданные_НайтиПодходящиеМетаданные(ОбщиеПараметры)
	...
	КлючКомандыВрег = ВРег(ПараметрыВыполнения.Ключ);
	КлючТипа = ОбщиеПараметры.СловарьПоиска.КлючиТипов.Получить(КлючКомандыВрег);
	ПустаяСтрокаПоиска = ПустаяСтрока(ПараметрыВыполнения.СтрокаПоиска);
	
	Если КлючТипа = Неопределено Тогда
		Если ПустаяСтрокаПоиска И КлючКомандыВрег <> ВРег("Все") Тогда
			Возврат;
		КонецЕсли;
