...
Процедура ПередЗаполнениемПараметровРасширенногоГлобальногоПоиска(Параметры) Экспорт
	...
	РГП_ГлобальныйПоискСерверПереопределяемый.Модуль()
		.ПередЗаполнениемПараметровРасширенногоГлобальногоПоиска(Параметры);
...
Функция ПараметрыВыполненияГлобальногоПоискаМетаданных(ИзначальнаяСтрокаПоиска, ПараметрыВыполнения) Экспорт
	...
	Результат.Вставить("ЭтоПолноправныйПользователь", 
		РГП_ГлобальныйПоискСерверПереопределяемый.МодульПользователей().ЭтоПолноправныйПользователь());
...
Функция СловарьГлобальногоПоискаМетаданных(ИзКэша = Истина) Экспорт
	...
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.БизнесПроцессы, "БизнесПроцессы", "БизнесПроцесс", Истина, Истина, "БП");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.Документы, "Документы", "Документ", Истина, Истина, "Док");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.ЖурналыДокументов, "ЖурналыДокументов", "ЖурналДокументов", Ложь, Ложь, "Журнал,ЖД,Документ");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.Задачи, "Задачи", "Задача", Истина, Истина, "Задачи,Задача");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.Обработки, "Обработки", "Обработка", Ложь, Ложь, "Обр");
		
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.Отчеты, "Отчеты", "Отчет", Ложь, Ложь, "Отчеты");
		
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.ПланыВидовРасчета, 
		"ПланыВидовРасчета", "ПланВидовРасчета", Истина, Истина, "ПВР,ПланВидаРасчета,ПланРасчета");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.ПланыВидовХарактеристик, 
		"ПланыВидовХарактеристик", "ПланВидовХарактеристик", Истина, Истина, "ПВХ,ПланВидаХарактеристик");
		
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.ПланыОбмена, "ПланыОбмена", "ПланОбмена", Истина, Истина, "ПО,Обмен");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.ПланыСчетов, "ПланыСчетов", "ПланСчетов", Истина, Истина, "ПС,Счет,Счета");
		
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.РегистрыБухгалтерии, "РегистрыБухгалтерии", "РегистрБухгалтерии", Истина, Ложь, "РБ,Бухгалтерия,БУ");
	
    ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.РегистрыНакопления, "РегистрыНакопления", "РегистрНакопления", Истина, Ложь, "РН");
	
    ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.РегистрыРасчета, "РегистрыРасчета", "РегистрРасчета", Истина, Ложь, "РР");
		
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.РегистрыСведений, "РегистрыСведений", "РегистрСведений", Истина, Ложь, "РС");
	
	ДобавитьМетаданныеВСловарь(Словарь, 
		Метаданные.Справочники, "Справочники", "Справочник", Истина, Истина, "Спр");
...
Функция МодульПользователей() Экспорт
	...
	Иначе
		...
		Возврат Вычислить("Пользователи"); // BSLLS:ExecuteExternalCodeInCommonModule-off
...
Процедура ДобавитьЗаписьЖР(Текст, Знач Уровень = "Ошибка") Экспорт
	...
	ЗаписьЖурналаРегистрации("РасширенныйГлобальныйПоиск", Уровень, , , Текст);
...
Процедура ЗаписатьЗначенияПользовательскихНастроек(ПользовательскиеНастройки) Экспорт
	...
	Хранилище.Сохранить(ПараметрыОтбора.КлючОбъекта, ПараметрыОтбора.КлючНастроек, ПользовательскиеНастройки);
...
Процедура ЗаполнитьПользовательскиеНастройкиВПараметрахГлобальногоПоиска(Параметры)
	...
	ОбработатьЗначенияПользовательскихНастроек(Параметры);
...
// Параметры:
//  Параметры - см. ПередЗаполнениемПараметровРасширенногоГлобальногоПоиска.Параметры
Процедура ОбработатьЗначенияПользовательскихНастроек(Параметры)
	
	ПользовательскиеНастройки = Параметры.ПользовательскиеНастройки.ОбщиеНастройки;
	ОписанияНастроек = Параметры.ОписанияПользовательскихНастроек;
	...
	Для Каждого КлючИЗначение Из ПользовательскиеНастройки Цикл
		...
		Если ЗначениеИзменилось Тогда
			ПользовательскиеНастройки[КлючНастройки] = ТекущееЗначение;
		КонецЕсли;
...
// * КлючОбъекта - Строка - 
// * КлючНастроек - Строка - 
Функция ПараметрыОтбораХранилищаПользовательскихНастроек()
...
Процедура ГлобальныйПоискМетаданные_НайтиПодходящиеМетаданные(ОбщиеПараметры)
	...
	МаксимумСтрокРезультата = ПараметрыВыполнения.ПользовательскиеНастройки.МаксимумСтрокРезультата;
	
	Для Каждого СтрокаСловаря Из ДанныеСловаря Цикл
		
		Если ПустаяСтрокаПоиска Тогда
			Если ПодходящиеМетаданные.Количество() >= МаксимумСтрокРезультата Тогда
				Возврат;
			Иначе
...
Процедура ГлобальныйПоискМетаданные_ЗаполнитьРезультатыПоиска(ОбщиеПараметры, РезультатыПоиска)
	...
	ПараметрыСтраниц = РГП_ГлобальныйПоискКлиентСервер.ПараметрыСтраницРезультатаКоманды(
		ПараметрыВыполнения, ПодходящиеМетаданные.Количество());
	ОсталосьВывести  = ПараметрыСтраниц.ОсталосьВывести;
	Если ОсталосьВывести = 0 Тогда
		Возврат;
	КонецЕсли;
	
	РГП_ГлобальныйПоискКлиентСервер.ДобавитьЭлементРезультатаСтандартнойКоманды_Страница(
	...
	Для Каждого СтрокаМетаданных Из ПодходящиеМетаданные Цикл
		...
		Если ПараметрыСтраниц.ВыводитьСоСтроки < НомерСтроки Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаСловаря.ЕстьПравоПоиска Тогда
			...
			ЭлементРезультата.Действия.Добавить(ВыполняемоеДействие, "Найти");
		...
		Если СтрокаСловаря.ЭтоТаблицаДанных И ОбщиеПараметры.ЭтоПолноправныйПользователь Тогда
			...
			ЭлементРезультата.Действия.Добавить(ВыполняемоеДействие, "Анализ данных");
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ЭлементРезультата.Действия) Тогда
			ЭлементРезультата.Действия.Вставить(0, СтрокаСловаря.СсылкаСписка, "Открыть");
		КонецЕсли;
		
		ГлобальныйПоискМетаданные_ЗаполнитьРезультатыПоиска_ВариантыОтчета(ЭлементРезультата, СтрокаСловаря);
...
Процедура ГлобальныйПоискМетаданные_ЗаполнитьРезультатыПоиска_ВариантыОтчета(ЭлементРезультата, СтрокаСловаря)
	
	ИмяОсновнойФормы = СтрШаблон("%1.Форма", СтрокаСловаря.ПолноеИмя);
	ЕдинственныйВариант = (СтрокаСловаря.ВариантыОтчетов.Количество() = 1);
	Для Каждого ВариантОтчета Из СтрокаСловаря.ВариантыОтчетов Цикл
		
		Если ЕдинственныйВариант 
			И ВРег(ВариантОтчета.Представление) = СтандартноеИмяОсновногоВариантаОтчета() Тогда
			Продолжить;
		КонецЕсли;
...
// * ВариантыОтчетов - Массив из ВариантНастроекКомпоновкиДанных - Варианты отчетов
Функция КоллекцияМетаданныхСловаряГлобальногоПоиска()
	...
	Результат.Колонки.Добавить("ВариантыОтчетов"  , Новый ОписаниеТипов("Массив"));
...
Процедура ДобавитьМетаданныеВСловарь(Знач Словарь, Знач КоллекцияМетаданных, 
	Знач ИмяКоллекции, Знач ИмяКоллекцииЕд, Знач ЭтоТаблицаДанных, Знач ЭтоСсылочныйТип, Знач СокращенияТипа = "")
	
	ЭтоПолныеПрава = РГП_ГлобальныйПоискСерверПереопределяемый.МодульПользователей().ЭтоПолноправныйПользователь();
...
Процедура ДобавитьМетаданныеВСловарь_ВариантыОтчетов(ОбъектМетаданных, НоваяСтрока)
	...
	ОсновнаяСхемаКомпоновкиДанных = ОбъектМетаданных.ОсновнаяСхемаКомпоновкиДанных;
	Если ОсновнаяСхемаКомпоновкиДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	МенеджерОтчета = Отчеты[ОбъектМетаданных.Имя];
	
	СхемаКомпоновкиДанных = МенеджерОтчета.ПолучитьМакет(ОсновнаяСхемаКомпоновкиДанных.Имя); // СхемаКомпоновкиДанных
	ВариантыОтчета = СхемаКомпоновкиДанных.ВариантыНастроек;
	Для Каждого ВариантОтчета Из ВариантыОтчета Цикл
		НоваяСтрока.ВариантыОтчетов.Добавить(ВариантОтчета);
	КонецЦикла;
...
// Возвращаемое значение:
//  Строка
Функция КлючевыеСловаПоискаОбъектаМетаданных(ОбъектМетаданных, СтрокаСловаря)
	...
	Для Каждого ВариантОтчета Из СтрокаСловаря.ВариантыОтчетов Цикл
		Для Каждого ИмяСвойстваВарианта Из СтрРазделить("Имя,Представление", ",") Цикл
			ТекущееЗначение = Строка(ВариантОтчета[ИмяСвойстваВарианта]);
