...
Функция МодульПользователей() Экспорт
	...
	ВозможныеИменаМодулей = "Пользователи,Users";
	Для Каждого ИмяМодуля Из СтрРазделить(ВозможныеИменаМодулей, ",") Цикл
		
		Если Метаданные.ОбщиеМодули.Найти(ИмяМодуля) <> Неопределено Тогда
			Возврат Вычислить(ИмяМодуля); // BSLLS:ExecuteExternalCodeInCommonModule-off
		КонецЕсли;
