...
// 
Процедура ДобавитьВИзбранное(ЭлементИзбранного) Экспорт
	
	ИзбранноеРаботы = ИзбранноеРаботыПользователя();
	
	НовыйЭлемент = Новый ЭлементИзбранногоРаботыПользователя();
	ЗаполнитьЗначенияСвойств(НовыйЭлемент, ЭлементИзбранного);
	ИзбранноеРаботы.Добавить(НовыйЭлемент);
...
// 
Процедура УдалитьИзИзбранного(НавигационнаяСсылка) Экспорт
	...
	Для Каждого Элемент Из УдаляемыеЭлементы Цикл
		Индекс = ИзбранноеРаботы.Индекс(Элемент); // Число
		ИзбранноеРаботы.Удалить(Индекс);
	КонецЦикла;
...
Процедура ЗаполнитьИзбранноеВПараметрахГлобальногоПоиска(Параметры)
	...
	Для Каждого Элемент Из Избранное Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
			...
			Параметры.Избранное.Вставить(ВРег(СтрокаПоиска), ЭлементИзбранного);
		КонецЕсли;
...
Функция ПараметрыОтбораХранилищаИзбранного()
	...
	Результат = Новый Структура;
	Результат.Вставить("КлючОбъекта", "Общее/ИзбранноеРаботыПользователя");
