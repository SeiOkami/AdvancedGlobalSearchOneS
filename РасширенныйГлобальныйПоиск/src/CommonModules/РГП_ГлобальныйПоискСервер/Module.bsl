...
Процедура ПередЗаполнениемПараметровРасширенногоГлобальногоПоиска(Параметры) Экспорт
	...
	ПараметрыСобытия.Добавить(Параметры);
	РГП_ГлобальныйПоискКлиентСервер.ВыполнитьПереопределяемоеСобытие(
		"ПередЗаполнениемПараметровГлобальногоПоиска", Ложь, Истина, ПараметрыСобытия);
...
// 
// Возвращаемое значение:
//  Массив из Строка - Имена переопределяемых общих модулей
