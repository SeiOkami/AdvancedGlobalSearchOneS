...
Процедура ГлобальныйПоискИсторияВычисленияВыражений(Знач СтрокаПоиска, РезультатыПоиска, ПараметрыВыполнения) Экспорт
	...
	Пока ВыборкаИстории.Следующий() Цикл
		...
		Представление = ЗначениеИстории.Выражение;
		Если СтрДлина(Представление) > МаксДлинаПредставления Тогда
			Представление = Лев(Представление, МаксДлинаПредставления) + "...";
		КонецЕсли;
