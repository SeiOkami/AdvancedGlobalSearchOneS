...
Функция ЭтоПолноправныйПользователь() Экспорт
	...
	Для Каждого ОписаниеМодуля Из ВозможныеМодули Цикл
		Если Метаданные.ОбщиеМодули.Найти(ОписаниеМодуля.ИмяМодуля) <> Неопределено Тогда
			ТекстКода = СтрШаблон("%1.%2()", ОписаниеМодуля.ИмяМодуля, 
				ОписаниеМодуля.Методы.ЭтоПолноправныйПользователь);
			//@skip-check server-execution-safe-mode - в данном случае нет необходимости
			// BSLLS:ExecuteExternalCodeInCommonModule-off
			Результат = Вычислить(ТекстКода); // Булево
...
Функция ВозможныеМодулиПользователи()
	...
	ВозможныеМодули.Добавить(
		ОписаниеВозможногоМодуляПользователи("Пользователи", "ЭтоПолноправныйПользователь"));
	ВозможныеМодули.Добавить(
		ОписаниеВозможногоМодуляПользователи("Users", "IsFullUser"));
...
Функция ОписаниеВозможногоМодуляПользователи(ИмяМодуля, МетодЭтоПолноправныйПользователь)
